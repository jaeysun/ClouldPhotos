package com.jaesun.cloudphotos.base
...
internal fun BasePager.replace(pageName: String) {
    try {
        if (pageData.isIOS) {
            this.acquireModule<BridgeModule>(BridgeModule.MODULE_NAME).replacePage(pageName)
        } else {
            this.acquireModule<RouterModule>(RouterModule.MODULE_NAME).openPage(pageName)
            this.acquireModule<RouterModule>(RouterModule.MODULE_NAME).closePage()
        }
    } catch (e: Exception) {
        KLog.e("replace", e.stackTraceToString())
    }
}